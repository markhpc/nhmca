package DataCollector::CollectlRSSMemUtilization;
use strict;
use warnings;
...
sub _executeQuery {
  ...
  my $query = "SELECT node,timestamp,vmrss from collectlprocessdata WHERE system='" . $cluster . "' AND timestamp >= '$start' AND timestamp < '$end'";
#  print "$query\n";
